    ...
    <script>
        ...
        function initToolbar() {
            ...
            filterClear.addEventListener('click', () => {
                Livewire.dispatch('clear-filters');
            });

            const filterCheckboxes = document.querySelectorAll('.filter-item input[type="checkbox"]');
            filterCheckboxes.forEach(checkbox => {
                checkbox.addEventListener('change', (e) => {
                    const category = e.target.closest('.filter-category').querySelector('.filter-category-title').textContent.trim();
                    const value = e.target.nextElementSibling.textContent.trim();
                    Livewire.dispatch('change-filter', category, value, e.target.checked);
                });
            });
        }
    </script>
