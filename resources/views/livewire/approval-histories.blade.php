            ...
            <div class="filter">
                ...
                <div class="filter-items-holder glass">
                    ...
                    @foreach ($filters as $category => $filter)
                        <div class="filter-category" wire:key="filter_hist_{{ $category }}">
                            ...
                            <div class="filter-items">
                                @foreach ($filter as $item)
                                    <div class="filter-item nos"
                                        wire:key="filter_hist_{{ $category }}_{{ $item }}"
                                        x-data="{
                                            category: '{{ $category }}',
                                            item: '{{ $item }}',
        ...
        <table class="svcr-table">
                ...
                <tr class="svcr-list-header">
                    ...
                    @foreach ($headers as $index => $header)
                        @php
                            ...
                            $label = $header['label'];
                        @endphp
                        <th class="svcr-list-header-item" data-column="{{ $column }}" wire:key="approval_history_header_{{ $index }}">
            ...
            <tbody>
                @forelse ($histories as $history)
                    <livewire:templates.approval-list-item :approval="$history" :type="'history'" wire:key="'approval_history_item_{{ $history->id }}'" :headers="$headers" :options="$itemOptions"/>
                @empty
                    <tr class="svcr-list-item">
                        <td class="svcr-list-item-cell" colspan="100%"">No approval histories found.</td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
